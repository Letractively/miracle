...
class mysqlrelation{
	...
	public function __construct($config=array(),$obj){
		
		if(isset($config['_model_name'])){
			$this->_model_name =$config['_model_name'];
		}
		if(isset($config['_tablename'])){
			$this->_tablename =$config['_tablename'];
		}
		if(isset($config['_validate'])){
			$this->_validate =$config['_validate'];
		}
		...
		if(is_array($this->_validate)) {
			$this->_validate_obj = new validate($this->_validate,$this->_validate_msg_box,$obj); //定义报警器
	 ...
	 */
	private function  getDbTableColumns($db,$tablename){
